 ...
 *  USA.
 *
 *  $Id: invoicenew.php,v 1.56.2.3 2008/01/04 07:58:11 alec Exp $
 */
...
$layout['pagetitle'] = trans('Nowa faktura kosztowa');
...
switch($action)
{
	case 'init':
		...
		$invoice['cdate']   = time();
		$invoice['paytime'] = 14;
		if(isset($_GET['customerid']) && $_GET['customerid'] != '' && $LMS->DostawcaExists($_GET['customerid']))
	...
	case 'additem':
		...
		if($itemdata['count'] > 0 && $itemdata['name'] != '')
		{
			...
			if($itemdata['valuenetto'] != 0)
			{
				...
				$itemdata['valuebrutto'] 	= f_round($itemdata['valuenetto'] * ($taxvalue / 100 + 1),2);
			...
			{
				$itemdata['valuebrutto'] 	= f_round($itemdata['valuebrutto']);
				$itemdata['valuenetto']  	= f_round($itemdata['valuebrutto'] / ($taxvalue + 100) * 100, 2);
				$itemdata['s_valuenetto'] 	= f_round($itemdata['valuenetto'] * $itemdata['count']);
	...
	case 'setcustomer':
		...
		if($invoice['number'])
		{
			if($DB->GetOne('select 1 from documents where type ='.DOC_DELIV_INVOICE.' and extnumber ='.$invoice['number']))
				$error['number'] = trans('Invoice number $a already exists!', $invoice['number']);
		} else {
			$error['number'] = 'Podaj numer dokumentu';
		}
	...
	case 'save':
		...
		if($contents && $customer)
		{
			...
			if($invoice['number'])
			{
				if($DB->GetOne('select 1 from documents where type =? and extnumber =?',array(DOC_DELIV_INVOICE, $invoice['number'])));
					$error['number'] = trans('Invoice number $a already exists!', $invoice['number']);
			} else {
				$error['number'] = 'Podaj numer dokumentu';
			}
			...
			$iid = $LMS->AddDelivererInvoice(array('customer' => $customer, 'contents' => $contents, 'invoice' => $invoice));
...
if($action)
{
	...
	$SESSION->redirect('?m=nm_fv_kosztowa_dodaj');
}
...
$SMARTY->display('nm_fv_kosztowa_dodaj.html');
