...
function Traffic($from = 0, $to = 0, $net = 0, $customerid = 0, $order = '', $limit = 0)
{
	...
	// period
	$fromdate = intval(time() - $from*60*60);
	$todate   = intval($to);
	$delta    = ($todate-$fromdate) ? ($todate-$fromdate) : 1;

	$dt = ($CONFIG['phpui']['radius'] == YES) ? 
	"(unix_timestamp(AcctStartTime) >= $fromdate AND unix_timestamp(AcctStartTime) < $todate )" : "( dt >= $fromdate AND dt < $todate ) ";
	...
	$query = "SELECT SUM( AcctOutputOctets ) AS download, 
	                 SUM( AcctInputOctets ) AS upload,
	                 UserName as name,
	                 FramedIPAddress as ip
			  FROM radacct
			 WHERE (DATE_SUB( NOW() , INTERVAL ? HOUR ) < AcctStopTime
					OR AcctStopTime =0)
			 GROUP BY FramedIPAddress,UserName
			 ORDER BY download DESC";
	
/*	
	
	'SELECT nodeid, name, inet_ntoa(ipaddr) AS ip, 
...
*/
	...
	$traffic = $DBRADIUS->GetAll($query,array($from));
...
$SMARTY->display('rad_traffic.html');
