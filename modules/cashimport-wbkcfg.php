 ...
 */
/*
$pattern = "/([^\|]+)\|([^\|]+)\|([^\|]+)\|([^\|]+)\|.*?109000049067([^\|]+)\|([^\|]+)/";

$pid 				= 5;		// customer ID position in expression
...
$encoding 			= 'cp1250';  // imported data encoding (for conversion)
...
$patterns[] = array(
    ...
    'ptnr' => 1,		// Neter sgt - identyfikator operacji
    'ptnr_hash' => FALSE,	// Neter sgt
    'ptnr_date_hash' => TRUE,	// Neter sgt
	'K_pattern' => "/K\|([^\|]+)\|([^\|]+)\|([^\|]+)\|([^\|]+)\|([^\|]+)/",
	'K_pdate' => 2,
);
