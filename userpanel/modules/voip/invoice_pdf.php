 ...
 *  USA.
 *
 *  $Id: invoice_pdf.php,v 1.13.2.1 2006/01/16 09:49:58 lexx Exp $
 */
...
function invoice_title($x,$y) 
{
    ...
    $tmp = docnumber($invoice['number'], $invoice['template'], $invoice['cdate']);
    if($invoice['invoice'])
	$y=$y-text_align_left($x,$y,$font_size,'<b>'.iconv("UTF-8","ISO-8859-2",trans('Credit Note No. $a',$tmp)).'</b>');
    else
	...
	$y=$y-text_align_left($x,$y,$font_size,'<b>'.iconv("UTF-8","ISO-8859-2", trans('for Invoice No. $a', $tmp)).'</b>');
	$y -= 5;
    }
    ...
    */
		    if($invoice['invoice'])
    {
	    ...
	    $tmp = docnumber($invoice['invoice']['number'], $invoice['invoice']['template'], $invoice['invoice']['cdate']);
	//$y=$y-
			text_align_left($x,$y,$font_size,'<b>'.iconv("UTF-8","ISO-8859-2//TRANSLIT",trans('for Invoice No. $a',$tmp)).'</b>');
	//$y -= 5;
    }
    
    //$font_size = 16;
    $y=$y-text_align_right(550,800,$font_size,iconv("UTF-8","ISO-8859-2//TRANSLIT",$type));
    if($invoice['invoice']) $y += 10;
...
function invoice_data($x,$y,$width,$font_size,$margin) 
{
	...
	$tt_width[5] = $pdf->getTextWidth($font_size,sprintf('%.2f',$item['count']));
...
function invoice_to_pay($x,$y) 
{
    global $pdf, $invoice;
    if($invoice['rebate'])
	$y = $y - text_align_left($x,$y,14,iconv("UTF-8","ISO-8859-2",trans('To repay:')).' '.iconv("UTF-8","ISO-8859-2",moneyf($invoice['total'])));
    else
...
function invoice_footnote($x, $y, $width, $font_size) 
{
    ...
    $pdf->line(0,$y,600,$y);
    $y-=10;
    text_align_left(100,$y,8,'<b>Pieczêæ firmy</b>');
    text_align_left(200,$y,8,'Imiê, nazwisko i podpis osoby uprawnionej');
    text_align_left(200,$y-8,8,'do wystawienia faktury VAT');
    text_align_left(400,$y,8,'Imiê, nazwisko i podpis osoby uprawnionej');
    text_align_left(400,$y-8,8,'do odbioru faktury VAT');
    text_align_left(400,$y-20,8,'<b>w/g o¶wiadczenia w umowie</b>');
...
{
    global $invoice,$pdf,$id,$CONFIG,$_SYSTEM_DIR;
...
/*	    $top=800;
	    ...
	    break;*/
		$top=800;
	    invoice_dates(450,780);    
	    $pdf->line(0,790,600,790);
  //  	    invoice_address_box(400,700);
  $pdf->line(0,745,600,745);
  $top=invoice_title(30,$top);
  $top=$top-40;
  $topold=$top;
  $top=invoice_seller(30,$top);
  $top=$top-20;
  invoice_buyer(300,$topold);
  $top=$top-20;
  $return=invoice_data(30,$top,530,7,2);
  invoice_expositor(30,$return[1]-20);
  $top=$return[2]-20;
  ...
  break;
	...
	default:
	    if(file_exists($template))
		    ...
		    require($_SYSTEM_DIR.'/modules/'.$template);
    }
    if (!($invoice['last'])) $id=$pdf->newPage(1,$id,'after');
}

global $_LIB_DIR,$layout,$pdf;
...
$pdf->addInfo('Producer','Userpanel Developers');
